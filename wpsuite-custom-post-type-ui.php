...
add_action( 'load_textdomain', function( $domain = '', $mofile = '' ) {
	if ( $domain === 'custom-post-type-ui' ) {
		...
		if ( $basedir === substr( $mofile, 0, $baselen ) ) {
			...
			if ( $authormo = trailingslashit( WP_PLUGIN_DIR ) . $langdir . 'custom-post-type-ui-ja.mo' ) {
				load_textdomain( $domain, $authormo );
				if ( ! file_exists( $mofile ) && ! stristr( $mofile, '/languages/loco/plugins/' ) ) {
					copy( $authormo, $mofile );
				}
			}
		}
	}
...
add_filter( 'site_transient_update_plugins', function() {
	if ( isset( $value ) && is_object( $value ) ) {
		unset( $value->response[ 'wpsuite-custom-post-type-ui/wpsuite-custom-post-type-ui.php' ] );
	}
} );
